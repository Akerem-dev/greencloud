  ...
  try {
    ...
    try {
      ...
      throw friendlyFirebaseError(
        readError,
        "The pairing claim could not be read after the write was rejected.",
      );
    }
...
function waitForClaimDecision(claimRef, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => {
      finish(() =>
        reject(
          new PairingFlowError("aborted", "Pairing was cancelled."),
        ),
      );
    };
...
export async function pairDeviceWithProtectedClaim(options) {
  ...
  const remainingMs = pairing.expiresAtMs - nowMs;
  ...
  const { claimRef, resumed } = await createOrResumeClaim(
    database,
    pairing,
    userId,
    nowMs,
